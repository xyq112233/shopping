...
export default {
  ...
  created () {
    if (this.$store.getters.token) {
      this.$store.dispatch('cart/getCartAction').then(() => {
      })
    } else {
  ...
  methods: {
    ...
    goPay () {
      ...
      // 使用 selCartList 或从 cartList 中过滤
      let selectedItems = []

      if (this.selCartList && Array.isArray(this.selCartList)) {
        selectedItems = this.selCartList
      } else {
        // 如果 selCartList 不可用，从 cartList 中过滤
        selectedItems = (this.cartList || []).filter(item => item && item.isChecked)
      }
      ...
      const cartIds = selectedItems
        .filter(item => item && item.id) // 确保 item 和 item.id 存在
        .map(item => item.id)
        .join(',')
